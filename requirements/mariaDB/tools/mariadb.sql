...
CREATE DATABASE IF NOT EXISTS wordpressDB; -- craete database if not exists (name: wordpress)
CREATE USER IF NOT EXISTS 'roudouch'@'%' IDENTIFIED BY '1234'; -- create user with password if not exists
GRANT ALL PRIVILEGES ON wordpressDB.* TO 'roudouch'@'%'; -- give all privileges to user 'roudouch'@'%' on database 'wordpress'
